...
class D_VECTOR(nn.Module):
    def __init__(self, num_layers=3, dim_input=80, dim_cell=768, dim_emb=16, seg_len=160):
    ...
    def embed_utterance(self, x):
        ...
        pad = torch.cat([x, torch.zeros([max(160 - x.shape[0], step - x.shape[0] % step), x.shape[1]]).to(x.device)], 0)
